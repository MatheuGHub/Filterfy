...
const filterfy = readline.question(`Please insert the full URL of the desired repository: `, (repo) => {
	...
	if(inputURL === `https://github.com`) {
		...
		let gitRepository = `https://api.github.com/repos/${validUserURL}/issues?=page=1&per_page=100`;
	//Uses the user's URL to fetch the info from the API and turns it into a JS object array
		async function getGitInfo() {
			let gitInfo;
			let fetchResponse = await fetch(gitRepository);
			gitInfo = await fetchResponse.text();
			let makeJsonObject = JSON.parse(gitInfo);
		//Filtering the content of the array. 
			let repositoryIssues = "title― body― state― number― assignee― labels― milestone\n";
			...
			//Organizes the array into readable CSV content content.
				repositoryIssues += finalData.title.replace(/"/g, "'");
				repositoryIssues += "―" + finalData.body.replace(/(\r\n|\n|\r)/gm, "").replace(/"/g, "'")
				repositoryIssues += "―" + finalData.state;
				repositoryIssues += "―" + finalData.number;
				repositoryIssues += "―" + assigneeLogin;
				repositoryIssues += "―" + JSON.stringify(finalData.labels); //Turns the "labels" vector into a JS string/object.
				repositoryIssues += "―" + milestoneTitle;
				repositoryIssues += "\n"
			});
